...
・ユーザーの設定の値の取り方
・augroup、autocmdの使い方
・実際には存在しない、"head;sample.txt"、"tail;sample.txt"という名前のファイルへの操作をautocmdで横取りする。
・"set nomodified"で、編集中の状態をリセット
・"filetype detect"で、ファイルタイプを判定できる
・ユーザに質問を投げて、その結果で処理を変える
